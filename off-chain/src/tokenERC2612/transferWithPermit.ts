...
async function transferWithPermitERC2612(amount: string) {
  if (!amount) {
    console.error('Usage: pnpm transfer-with-permit-erc2612 <amount>')
    process.exit(1)
  }
  ...
  const from = SIGNER_ACCOUNT.address
  const to = RELAYER_ACCOUNT.address
  ...
  const PERMIT_TYPE = 'TransferWithPermit'
  ...
  const permitTx = await relayerWalletClient.writeContract({
    ...
    args: [from, to, weiAmount, deadline, permitSignature]
  })

  await relayerWalletClient.waitForTransactionReceipt({ hash: permitTx })
  console.log('Transfer with permit transaction sent:', permitTx)
  console.log(`${SIGNER_ACCOUNT.address} approved ${weiAmount} tokens for ${RELAYER_ACCOUNT.address} with transfer with permit signature.`)
}

const args = process.argv.slice(2)
const amount = args[0]
transferWithPermitERC2612(amount).catch((error) => {
  console.error('Error executing transfer with permit ERC2612:', error)
  process.exit(1)
})
